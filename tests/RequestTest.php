...
final class RequestTest extends TestCase {
    ...
    public function test_getUrl() : void {
        ...
        // Return full URL
        $this->assertSame('http://example.com/foo/bar?foo=1#bar', $method->invoke(new Request('get', 'http://example.com/foo/bar?foo=1#bar')));
    }
    ...
    public function test_getPath() : void {
        ...
        $method->setAccessible(true);
        // Return path
        $this->assertSame('foo/bar', $method->invoke(new Request('get', 'http://example.com/foo/bar?foo=1#bar')));
        // Return empty path
        $this->assertSame('', $method->invoke(new Request('get', 'http://example.com/?foo=1#bar')));
        $this->assertSame('', $method->invoke(new Request('get', 'http://example.com?foo=1#bar')));
    }
    ...
    public function test_getQuery() : void {
        ...
        $method->setAccessible(true);
        // Return query
        $this->assertSame('foo=1&bar=2', $method->invoke(new Request('get', 'http://example.com/foo/bar?foo=1&bar=2#bar')));
        // Return empty query
        $this->assertSame('', $method->invoke(new Request('get', 'http://example.com/foo/bar?#bar')));
        $this->assertSame('', $method->invoke(new Request('get', 'http://example.com/foo/bar#bar')));
    }
